...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($_POST['confirmnewpassword'])) {
		...
		if (strlen($_POST['confirmnewpassword']) >= 4) {
			...
			if ($newpassword != $confirmnewpassword) {
				...
				if ($rowCount < 1) {
					...
					while ($row = mysqli_fetch_assoc($result)) {
						...
						if ($oldpassword== $uPassInDB) {
							$sql = "UPDATE login SET password='$confirmnewpassword' WHERE username='$username'";
...
<body>
	<div class="loginbox">
		...
		<form action="changepassword.php" method="POST">
			<p>Old Password</p>
			<input type="password" name="oldpassword" placeholder="Enter Old Password"><br><span><?php echo $oldpasswordErr ?></span>
			<p>New Password</p>
			<input type="password" name="newpassword" placeholder="Enter New password"><br><span><?php echo $newpasswordErr ?></span>
			<p>Confirm New Password</p>
			<input type="password" name="confirmnewpassword" placeholder="Confirm New password"><br><span><?php echo $confirmnewpasswordErr ?><?php echo $notmatchErr ?></span>
